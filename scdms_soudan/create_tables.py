...
def find_valid_series_events(cut_data_file, is_test=True):
    """
    Given a .csv cut data file, return two dictionaries,
    one of True series-event pairs and another of False series-event pairs
    """
    # Load cdms id file
    id_path = '/data3/afisher/cdmslite-run3-cuts-output/ID_CDMSliteR3.csv'
    ...
    if is_test:
        # For testing
        cdms_ids = cdms_ids_small

    # Load cut_data_file
    cut_data_file = pd.read_csv(cut_data_file, header=None)
    cut_data_file.columns = ['bool']

    # Filter for valid rows (where 'bool' = 1)
    valid_rows = cdms_ids.loc[cut_data_file['bool'] == 1]
    invalid_rows = cdms_ids.loc[cut_data_file['bool'] == 0]
...
#cut_data_file = '/data3/afisher/cdmslite-run3-cuts-output/cut_output_bg-restricted_IsGlitch_chisq_CDMSliteR3_small.csv'
valid_pairs = find_valid_series_events(cut_data_file, is_test=False)
